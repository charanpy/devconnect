...
const handleCatalystDuplicate = () => {
  return new AppError('Invalid Credentials', 400);
};

const handleAuthCatalyst = () => {
...
const errorController = (err, req, res, next) => {
  console.log(err);
  console.log(err?.code, err?.message, typeof err);
  ...
  const catalystError = err?.split?.('code : ')?.[1];
  if (catalystError?.startsWith?.('DUPLICATE_ENTRY')) {
    error = handleCatalystDuplicate();
  }
